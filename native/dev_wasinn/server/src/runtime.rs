...
impl WasmInstance
{
    pub fn new(engine: Arc<Engine>, module: Arc<Module>, wasm_module_path: String) -> anyhow::Result<WasmInstance>
    {
        let full_path: PathBuf = Path::new(&wasm_module_path).parent().unwrap().join("fixture");
        let mut store =
            Store::new(&engine, Context::new(&full_path, true, Backend::from(OpenvinoBackend::default())).unwrap());
