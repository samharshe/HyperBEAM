...
pub async fn start_server(port: u16, wasm_module_path: String) -> anyhow::Result<()>
{
    ...
    tokio::spawn(async move {
        ...
        let p = wasm_module_path.clone();
        let module = Arc::new(Module::from_file(&engine, Path::new(&wasm_module_path)).unwrap());
        while let Some(request) = rx.recv().await {
            let p_clone = p.clone();
            let engine = Arc::clone(&engine);
            let module = Arc::clone(&module);
            spawn_blocking(move || -> anyhow::Result<()> {
                let mut instance = WasmInstance::new(engine, module, p_clone)?;
