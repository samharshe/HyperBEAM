...
// === SERVER URLS ===
const textServerURL = "http://127.0.0.1:3001";
const imageServerURL = "http://127.0.0.1:3000";
...
function openTextSSE() {
    closeCurrentSSE();
    // Text mode creates SSE connections per chat submission - no persistent connection needed
}

function openImageSSE() {
    closeCurrentSSE();
    currentEventSource = new EventSource(imageServerURL + "/logs");
...
chatForm.addEventListener('submit', async function(e) {
    ...
    const streamEventSource = new EventSource(textServerURL + "/logs");
    
    // Cleanup function to ensure SSE is properly closed
    const cleanupStreamSSE = () => {
        if (streamEventSource && streamEventSource.readyState !== EventSource.CLOSED) {
    ...
    try {
        const response = await fetch(`${textServerURL}/infer`, {
            method: 'POST',
            headers: {
...
// ImageNet labels for MobileNet classification
...
document.querySelectorAll('.gallery img').forEach(img => {
    img.addEventListener('click', async function() {
        ...
        try {
            ...
            const serverResponse = await fetch(imageServerURL + "/infer", {
                method: 'POST',
                headers: {
